...
// ==================== ИНДИКАЦИЯ ЗА АКТИВЕН ПОРТАЛ ВЪРХУ БУТОНА "ЕКСПЕДИЦИИ" ====================
(function() {
    function initPortalIndicator() {
        const expeditionsBtn = document.getElementById('btnExpeditions');
        if (!expeditionsBtn) {
            console.warn("Бутонът за експедиции не е намерен");
            return;
        }
        
        // Създаване на индикатора (червена точка)
    ...
    // Функции за показване/скриване на индикатора
    window.showPortalIndicator = function() {
        ...
        console.log("🔴 Индикатор за портал АКТИВИРАН");
    };

    window.hidePortalIndicator = function() {
        const ind = document.getElementById('portal-indicator');
        if (ind) ind.style.display = 'none';
        console.log("🔴 Индикатор за портал ДЕАКТИВИРАН");
    };

    // Автоматично скриване при битка с портал (patch на startBattle)
    const originalStartBattle = window.startBattle;
    if (originalStartBattle) {
        window.startBattle = function(regionInput) {
            // Проверка дали това е портал
            let isPortal = false;
            if (typeof regionInput === 'string' && regionInput.includes('Портал')) isPortal = true;
            if (typeof regionInput === 'object' && regionInput.id && regionInput.id.includes('portal')) isPortal = true;
            if (typeof regionInput === 'object' && regionInput.name && regionInput.name.includes('Портал')) isPortal = true;
            
            if (isPortal) {
                window.hidePortalIndicator();
            }
            return originalStartBattle(regionInput);
        };
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initPortalIndicator);
    } else {
        initPortalIndicator();
    }
})();
