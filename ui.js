 ...
 * СТАТУС: ФИНАЛНА СИНХРОНИЗАЦИЯ И НАДГРАЖДАНЕ
 * Поправка на времето, сезоните и еволюцията на династиите.
 */
...
window.updateCharacterUI = function(hero) {
    ...
    if (leftSidebar) {
        ...
        // Поправка: добавяме възрастта към основния изглед
        let treeHTML = `
            ...
            <div style="margin-bottom: 20px;">
                ...
                <div style="font-size: 0.9em; max-height: 200px; overflow-y: auto; background: rgba(0,0,0,0.3); padding: 5px;">
                    ${Object.keys(window.activeDynasties || {}).map(clanName => {
                        ...
                        const regions = clan.regionsOwned || 0;
                        return `
                            <div style="display: flex; justify-content: space-between; margin-bottom: 4px; color: ${isPlayer ? '#d4af37' : '#fff'}">
                                ...
                                <span style="color: #888; font-size: 0.8em;">${regions} зем.</span>
                            </div>
                        `;
                    }).join('')}
                </div>
            ...
            <div>
                <h4 style="color: #d4af37; border-bottom: 1px solid #444; padding-bottom: 5px; font-family: 'Cinzel', serif;">НАШИТЕ ЗЕМИ (${window.playerRegions.length})</h4>
    ...
    // ПОПРАВКА: Синхронизиране на ID-тата с index.html
...
window.showAdvisorMsg = function(msg) {
    ...
    // Веднага обновяваме интерфейса
    window.updateCharacterUI(window.currentHero);
    window.updateNotificationBadge();
};

/**
 * ОБНОВЯВАНЕ НА БАДЖА ЗА НОВИ СЪБИТИЯ
 */
...
// Извикваме UI опресняване при зареждане, за да премахнем "Зареждане..."
