...
(function() {
    ...
    // Затваряме всички модали на карти
    ...
    // ========== ДОБАВЯНЕ НА БОНУС ОТ ГИЛДИЯТА НА ВОИНИТЕ ==========
    let attackBonus = window.getGuildBonus ? window.getGuildBonus('attackBonus') : 0;
    if (attackBonus > 0) {
        for (let hero of playerHeroes) {
            if (hero.power) hero.power += attackBonus;
        }
    }
    // ================================================================
    ...
    updateUI();
    console.log("✅ Битката е готова (с подкрепления от нелюбими герои и бонус от гилдия)!");
}
