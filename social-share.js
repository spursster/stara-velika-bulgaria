...
window.ShareUtils.generateHeroTikTokVideo = async function(hero) {
  ...
  function drawFrame(now) {
    ...
    // Декорация
    ctx.fillStyle = `rgba(100, 200, 255, ${0.05 + t * 0.15})`;
    ctx.beginPath(); ctx.arc(540, 400 + Math.sin(t * Math.PI * 2) * 15, 350, 0, Math.PI * 2); ctx.fill();

    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.globalAlpha = 1;

    // Име
    if (t > 0.1) {
      const p = easeOut(Math.min((t - 0.1) / 0.3, 1));
      ctx.globalAlpha = p;
      ctx.font = 'bold 84px system-ui, sans-serif'; ctx.fillStyle = '#fff';
      ctx.fillText(`🏛️ ${hero.name || 'Безименен Хан'}`, 540, 500 + 150 * (1 - p));
    }

    // Статистики
    if (t > 0.35) {
      const p = easeOut(Math.min((t - 0.35) / 0.4, 1));
      ctx.globalAlpha = p;
      ctx.font = '58px system-ui, sans-serif'; ctx.fillStyle = '#d0d0e0';
    ...
    // CTA
    if (t > 0.75) {
      const p = easeOut(Math.min((t - 0.75) / 0.25, 1));
      ctx.globalAlpha = p;
      ctx.font = '62px system-ui, sans-serif'; ctx.fillStyle = '#25F4EE';
      ctx.fillText('🔗 Линк в Bio', 540, 1460 + 40 * (1 - p));
      ctx.font = '40px system-ui, sans-serif'; ctx.fillStyle = '#8888aa';
      ctx.fillText('spursster.github.io/stara-velika-bulgaria', 540, 1540);
      ctx.font = '36px system-ui, sans-serif'; ctx.fillStyle = '#666688';
      ctx.fillText('#ВеликаБългария #Стратегия #IndieGameBG', 540, 1690);
    }
