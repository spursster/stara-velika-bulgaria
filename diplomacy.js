...
window.openMarriageMenu = function(clan) {
    ...
    // ИЗПОЛЗВАМЕ СЪВЕТНИКА ЗА ВЕЧЕ СКЛЮЧЕН БРАК
    if (window.currentSpouse) { 
        window.showAdvisorMsg("Велики Кане, Вие вече сте сключили свещен династичен съюз! Не подобава на владетел да нарушава клетвата си.");
        return; 
    }

    // ИЗПОЛЗВАМЕ СЪВЕТНИКА ЗА НИСКО ДОВЕРИЕ
    if (window.clanRelations[clan] < 60) { 
        window.showAdvisorMsg(`Родът ${clan} все още не ни се доверява достатъчно. Трябват ни поне 60% доверие, за да приемат предложението ни за брак!`);
        return; 
    }
    ...
    // СЪВЕТНИКЪТ СЪОБЩАВА ЗА УСПЕШНИЯ БРАК
    window.showAdvisorMsg(`Поздравления, Велики Кане! Сключен е съюз с род ${clan}. Като зестра получаваме владението над ${region}.`);
