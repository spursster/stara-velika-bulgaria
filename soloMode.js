// ==================== СОЛО РЕЖИМ – ВЕРСИЯ 3.1 (НАПЪЛНО ФУНКЦИОНАЛЕН) ====================
(function() {

    // ==================== ГЛОБАЛНИ НАСТРОЙКИ ====================
    window.soloSettings = window.soloSettings || {
        showNeighborsOnMap: true,
        questChance: 0.3,
    ...
    if (!window.visitedRegions) window.visitedRegions = new Set();

    // ==================== ОСНОВНА ИНИЦИАЛИЗАЦИЯ ====================
    function initSoloMode() {
        if (window.gameMode !== 'solo') return;
        console.log("🌍 Инициализация на соло режим 3.1");
    ...
    function addQuestsButton() {
        if (window.gameMode !== 'solo') return;
        let container = document.querySelector('#sidebar-right .command-buttons');
        if (!container) {
    ...
    window.updateRegionIndicator = updateRegionIndicator;
    window.addQuestsButton = addQuestsButton;
    window.addSoloSettingsButton = addSoloSettingsButton;
    window.initSoloMode = initSoloMode;

    // Ако travelToRegion не е дефинирана (резерв), дефинираме я
...
// Автоматично извикване на бутоните, ако вече сме в соло режим
if (window.gameMode === 'solo' && document.readyState === 'complete') {
    setTimeout(() => {
        if (typeof window.addQuestsButton === 'function') window.addQuestsButton();
        if (typeof window.addSoloSettingsButton === 'function') window.addSoloSettingsButton();
    }, 500);
}
