...
СТАТУС: НАПЪЛНО СИНХРОНИЗИРАН С UNLOCKEDLEADERS И ЕЛИТНАТА ЛЕНТА
КОРЕКЦИЯ: Поправени всички разделени думи, оператори и arrow функции. Унифицирано към unlockedLeaders & clan.
*/
...
window.renderBarracksLayout = function() {
    ...
    // 1. Извличане САМО на отключените от играча реални герои (унифицирано към unlockedLeaders)
        ...
        if (hero) {
            ...
            topSlotsHTML += `
                <div class="elite-hero-card" style="background: rgba(212, 175, 55, 0.1); border: 2px solid #d4af37; border-radius: 8px; width: 18%; padding: 10px; text-align: center; position: relative; box-sizing: border-box; display: flex; flex-direction: column; justify-content: space-between; min-height: 85px;">
                    ...
                    <div style="font-size: 11px; font-weight: bold; color: #ffd700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding-right: 15px;">👑 ${hero.name}</div>
...
window.buyUnits = function() {
    ...
    // Синхронизираме в глобалната карта на клановете
    if (window.worldData && window.worldData.clans && window.worldData.clans[window.currentHero.clan]) {
        ...
        window.worldData.clans[window.currentHero.clan].currentArmy = window.currentHero.currentArmy;
    }
