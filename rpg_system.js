// =========================================================================
// ВЕЛИКА БЪЛГАРИЯ - rpg_system.js (ПЪЛЕН, КОРИГИРАН)
// =========================================================================
...
window.rpgDatabase.skillTrees = {
    ...
    raiseDead: { name: "Въздигане на падналите", desc: "Временно съживява част от падналите врагове." },
...
window.initializeHeroRPGData = function(leader) {
    ...
    leader.isAuto = true; // Всички герои се развиват автоматично
...
window.gainHeroXP = function(leader, amount) {
    ...
    if (leveledUp) window.checkArcheAgeClass(leader);
    ...
    if (skillsContainer) {
        ...
        if (!leader.isAuto) {
            ...
            if (leader.storedXP >= reqXP) {
                ...
                xpBarBtn.innerHTML = `<div>📊 Събран ръчен опит: <b>${leader.storedXP} / ${reqXP} XP</b> (не достига опит)</div>`;
            }
