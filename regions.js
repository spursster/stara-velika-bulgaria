...
КОРЕКЦИЯ: Премахнати синтактични грешки, използва безопасен метод за стартиране на битка
========================================================================== */ 
...
window.openRegionsMap = function() { 
    ...
    let html = `<div style="padding:20px;"><h2 style="color:#ffdd99;">Карта на Регионите</h2><div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(180px,1fr)); gap:12px;">`; 
    ...
    html += `</div><button style="margin-top:20px;" onclick="window.showMainMenu()">Назад към Главното Меню</button></div>`; 
...
window.inspectRegion = function(regionName) { 
    ...
    if (isPlayerOwned) { 
        actionButtonHTML = `<button class="region-action-btn" style="background:#2c5a2a;" onclick="window.upgradeRegionInfrastructure('${regionName.replace(/'/g, "\\'")}', ${finalUpgradeCost})">️ Модернизирай Инфраструктура (${finalUpgradeCost} зл.)️</button>`; 
    } else { 
        // Безопасен начин: data-атрибут, без JSON.stringify в onclick
        actionButtonHTML = `<button class="region-action-btn attack-region-btn" style="background:#7a2e1a;" data-region-name="${regionName.replace(/"/g, '&quot;')}">⚔️ Изпрати Войски за Завладяване ⚔️</button>`;
    } 
    ...
    // Закачаме event listener за бутона за атака, ако съществува
    const attackBtn = overlay.querySelector('.attack-region-btn');
    if (attackBtn) {
        attackBtn.addEventListener('click', function(e) {
            const rName = this.getAttribute('data-region-name');
            if (window.startBattle) {
                const targetRegion = window.worldData.regions[rName];
                if (targetRegion) {
                    window.startBattle(targetRegion);
                } else {
                    console.error("Регионът не е намерен:", rName);
                    if (window.showAdvisorMsg) window.showAdvisorMsg("Грешка: Регионът не може да бъде атакуван.");
                }
            } else {
                console.error("startBattle не е дефинирана");
                alert("Бойната система не е заредена! Проверете дали battle.js е зареден.");
            }
            // Затваряме инспекцията
            const ov = document.getElementById('region-inspect-overlay');
            if (ov) ov.remove();
        });
    }
