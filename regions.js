...
ФАЙЛ: regions.js (КАРТА, ИНСПЕКЦИЯ И ИНФРАСТРУКТУРА)
СТАТУС: НАПЪЛНО ИЗЧИСТЕН И СИНХРОНИЗИРАН
КОРЕКЦИЯ: Премахнати всички синтактични грешки, поправени счупени HTML низове, само clan.
==========================================================================
*/
window.openRegionsMap = function() {
    ...
    // ✅ ФИКС: Премахнат интервал от името на променливата
    ...
    regionKeys.forEach(key => {
        ...
        const borderStyle = isPlayerOwned ? "2px solid #00ffcc" : "1px solid #333";
        const bgStyle = isPlayerOwned ? "rgba(0, 255, 204, 0.05)" : "rgba(0,0,0,0.4)";
...
window.inspectRegion = function(regionName) {
    ...
    // ✅ ЧИСТИ HTML БУТОНИ БЕЗ РАЗБИТИ BACKTICKS
    let actionButtonHTML = '';
    if (isPlayerOwned) {
        actionButtonHTML = `<button onclick="window.upgradeRegionInfrastructure('${regionName}', ${finalUpgradeCost})" style="width: 100%; background: #00ffcc; color: #000; border: none; padding: 12px; font-weight: bold; cursor: pointer; text-transform: uppercase; border-radius: 4px; font-size: 0.85em; margin-bottom: 10px;">🏗️ Модернизирай Инфраструктура (${finalUpgradeCost} зл.)</button>`;
    } else {
        actionButtonHTML = `<button onclick="document.getElementById('region-inspect-overlay').remove(); if(window.startBattle) window.startBattle('${regionName}');" style="width: 100%; background: #ff3366; color: #fff; border: none; padding: 12px; font-weight: bold; cursor: pointer; text-transform: uppercase; border-radius: 4px; font-size: 0.85em; margin-bottom: 10px;">⚔️ Изпрати Войски за Завладяване</button>`;
    }
    ...
    overlay.innerHTML = `
        <div style="background: #0a0a0a; border: 2px solid #d4af37; width: 100%; max-width: 380px; border-radius: 8px; padding: 20px; color: white; box-sizing: border-box;">
            <h3 style="margin-top: 0; color: #ffd700; text-transform: uppercase; border-bottom: 1px solid #222; padding-bottom: 10px; text-align: center;">Инспекция: ${regionName}</h3>
            ...
            ${actionButtonHTML}
            <button onclick="document.getElementById('region-inspect-overlay').remove()" style="width: 100%; background: #222; color: #fff; border: 1px solid #444; padding: 10px; cursor: pointer; font-size: 0.8em; text-transform: uppercase; border-radius: 4px;">Затвори</button>
        </div>
    `;
